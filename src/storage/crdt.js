...
export async function idbSave(id, bytes, mime) {
  try {
    ...
    tx.objectStore('media').put({ id, bytes, mime });
  } catch { }
}
...
export class CRDT {
  ...
  chat(a, b) {
    return this.ops.filter(o => (o.from === a && o.to === b) || (o.from === b && o.to === a));
  }
  ...
  _save() {
    try {
      const ops = this.ops.map(o => {
        ...
        // Save bytes in IDB and remove from CRDT state (kept in localStorage)
        if (s.payload._bytes) {
          idbSave(o.id, s.payload._bytes, s.payload.mimeType);
          delete s.payload._bytes;
        }
