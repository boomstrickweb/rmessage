/**
 * sha3.js — FIPS 202: SHA3-256, SHA3-512, SHAKE-128, SHAKE-256
 *
 * Pure JavaScript implementation.
 * NIST test vectors verified.
 *
 * Exports: SHAKE128, SHAKE256, SHA3_256, SHA3_512
 */
...
function _sponge(rate, input, delim, outLen) {
  const rb = rate / 8;
  ...
  while (off < input.length) {
    ...
    for (let i = 0; i < bl; i++)
      st[Math.floor(i/8)] ^= BigInt(input[off + i]) << BigInt((i % 8) * 8);
    if (bl === rb) _kF(st);
    off += bl;
  }
  ...
  let op = 0;
  while (op < outLen) {
    for (let i = 0; i < rb && op < outLen; i++)
      out[op++] = Number((st[Math.floor(i/8)] >> BigInt((i % 8) * 8)) & 0xFFn);
    if (op < outLen) _kF(st);
  }
  return out;
}

export const SHAKE128  = (i, l) => _sponge(1344, i, 0x1F, l);
export const SHAKE256  = (i, l) => _sponge(1088, i, 0x1F, l);
export const SHA3_256  = i      => _sponge(1088, i, 0x06, 32);
export const SHA3_512  = i      => _sponge(576,  i, 0x06, 64);
